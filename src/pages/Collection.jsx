...
const Collection = () => {
  ...
  const initialSaleOnly = queryParams.get("sale_only") === "true";
  ...
  useEffect(() => {
    ...
    const isSale = params.get("sale_only") === "true";
  ...
  // دریافت اطلاعات محصولات از لاراول
  useEffect(() => {
  ...
  const updateURL = (cat, search, isSale) => {
    const params = new URLSearchParams();
    if (cat && cat !== "همه") params.set("category", cat);
    if (search.trim()) params.set("search", search.trim());
    if (isSale) params.set("sale_only", "true");
  ...
  const handleCategoryClick = (cat) => {
    setFilter(cat);
    updateURL(cat, searchTerm, saleOnly);
  };

  const handleSearchChange = (e) => {
    const val = e.target.value;
    setSearchTerm(val);
    updateURL(filter, val, saleOnly);
  };

  const handleSaleOnlyChange = (e) => {
    const isChecked = e.target.checked;
    setSaleOnly(isChecked);
    updateURL(filter, searchTerm, isChecked);
  };
          ...
          <div className="space-y-3">
            ...
            <details className="group border border-gray-200 rounded-xl p-3 bg-white shadow-sm">
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                />
                <label className="text-gray-700 text-sm cursor-pointer" onClick={() => handleSaleOnlyChange({ target: { checked: !saleOnly } })}>
                  فقط محصولات حراجی
                </label>
              </div>
            </details>
